...
#if defined(NDEBUG)
# error "Emojibit cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x1aa1925b322ff11618cfa809105fdd337cbcd9983b0f28b231878a34b6e6bbc5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Emojibit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Emojibit Signed Message:\n";
...
{
    // Emojibit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Emojibit: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Emojibit: 3.5 days
static const int64 nTargetSpacing = 1 * 30; // Emojibit: 30 secondss
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Emojibit: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Emojibit: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x346fcd07652f909e70fde96a69a40ee6991853f5223e8fd72be39c22327b8e84");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "The Start of EmojiBit Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 4000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04c364ba76d0ce910a3bdd0f3e04876a5570e1da5e040940594318a4dd8e2d125ec4fa2868709f51f286ed46352f12b32c487f8510f6ad3718ae8e4e7743fbe365") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1529443424;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084959981;

        if (fTestNet)
        {
            block.nTime    = 1529443412;
            block.nNonce   = 386262777;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x9c290511feb15fd946047f56df7363b8bdbbc74c70d3e6ef0dfcc290d8260502"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xbf, 0x0c, 0x6b, 0xbd }; // Emojibit: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EmojibitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EmojibitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EmojibitMiner(CWallet *pwallet)
{
    printf("EmojibitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("emojibit-miner");
    ...
    try { loop {
        ...
        printf("Running EmojibitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EmojibitMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EmojibitMiner, pwallet));
}
